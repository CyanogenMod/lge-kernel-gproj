...
static int ce1702_isp_event_store(struct device* dev, struct device_attribute* attr, const char* buf, size_t n)
{
	...
	if (val) {
		...
		if (num_read > 0) {
			...
			old_fs = get_fs();
			set_fs(KERNEL_DS);
			fd = sys_open("/data/eventlog.bin", O_RDWR | O_CREAT , 0644);
			if (fd >= 0) {
				sys_write(fd, dump_data, dump_data_len);
				sys_close(fd);
				LDBGE("%s: isp eventlog successfully writed (%d)!! \n", __func__, nloop);
			}
			set_fs(old_fs);
			kfree(dump_data);
...
static DEVICE_ATTR(firmware, S_IRUGO|S_IWUGO, ce1702_firmware_show, ce1702_firmware_store);
static DEVICE_ATTR(isp_event, S_IRUGO|S_IWUGO, ce1702_isp_event_show, ce1702_isp_event_store);
static DEVICE_ATTR(fps, S_IRUGO|S_IWUGO, ce1702_fps_show, NULL);
static DEVICE_ATTR(irq_log, S_IRUGO|S_IWUGO, ce1702_irq_log_show, ce1702_irq_log_store);
