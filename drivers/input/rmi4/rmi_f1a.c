...
static int rmi_f1a_alloc_memory(struct rmi_function_container *fc)
{
	...
	/*
	   ...
	   We set three buttons KEY_BACK,KEY_HOMEPAGE,KEY_MENU to platform data.
	   pdata->f1a_button_map->nbuttons should be equal to f1a->button_count
	*/
#if 1
	f1a->button_count = 3;
#else
...
int rmi_f1a_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	/* Move to Page2 */
	error = rmi_write(rmi_dev, 0xff, 0x02);
	...
	error = rmi_write(rmi_dev, 0xff, 0x00);
