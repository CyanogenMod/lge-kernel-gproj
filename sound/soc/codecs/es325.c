...
int es325_codec_sleep(void)
{
	...
	rc = msm_xo_mode_vote(xo_handle_a0, MSM_XO_MODE_OFF);
...
int es325_codec_wakeup(void)
{
	...
	rc = msm_xo_mode_vote(xo_handle_a0, MSM_XO_MODE_ON);
	if (IS_ERR(xo_handle_a0)) {
		rc = PTR_ERR(xo_handle_a0);
		pr_err("%s: Failed to get the msm_xo_mode_vote(%d)\n", __func__, rc);
		goto EXIT_WAKEUP;
...
{
	pr_info("%s:(dai->id = %d)(rx1_ena = %d)(tx1_ena = %d)(rx2_ena = %d)\n",
			__func__, dai->id,
...
static const char *es325_internal_route_configs_text[ES325_STOP+1] = {
	...
	"ONE MIC HEADSET",
	"ONE CH MUSIC PLAYBACK",	"TWO CH MUSIC PLAYBACK",
	"ONE MIC VOICE REC",
...
static __init int es325_init(void)
{
...
#else
	xo_handle_a0 = msm_xo_get(MSM_XO_TCXO_A0, "es325");
	if (IS_ERR(xo_handle_a0)) {
		ret = PTR_ERR(xo_handle_a0);
		pr_err("%s: Failed to get the xo_handle_a0(%d)\n", __func__, ret);
	}
	else {
		ret = msm_xo_mode_vote(xo_handle_a0, MSM_XO_MODE_ON);
		if (IS_ERR(xo_handle_a0)) {
			ret = PTR_ERR(xo_handle_a0);
			pr_err("%s: Failed to get the msm_xo_mode_vote(%d)\n", __func__, ret);
		}
	...
	if (ret) {
		...
		ret = msm_xo_mode_vote(xo_handle_a0, MSM_XO_MODE_OFF);
	}
#endif
