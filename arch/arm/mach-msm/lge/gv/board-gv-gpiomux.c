...
static struct gpiomux_setting maxim_hs_detect = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_2MA,
	.pull = GPIOMUX_PULL_UP,
};
...
#if !defined(CONFIG_MACH_LGE) || !defined(CONFIG_LGE_FELICA_NFC)
	{
		.gpio = 69,
		.settings = {
...
static struct gpiomux_setting mhl_int_n_cfg = {
    .func = GPIOMUX_FUNC_GPIO,
    .drv = GPIOMUX_DRV_8MA,
    .pull = GPIOMUX_PULL_UP,
    .dir = GPIOMUX_IN,
...
static struct msm_gpiomux_config apq8064_sii8334_mhl_configs[] __initdata = {
	/* MHL_INT_N */
	{
		.gpio = GPIO_MHL_INT_N_REV_C,
		.settings = {
			[GPIOMUX_SUSPENDED]	= &mhl_int_n_cfg,
		},
	},
};
...
#ifdef CONFIG_SWITCH_MAX1462X
static struct msm_gpiomux_config apq8064_headset_rev_b_configs[] __initdata = {
    /* GPIO_EAR_KEY_INT */
	{
		.gpio = 7,
		.settings = {
    ...
    /* GPIO_EAR_SENSE_N */
	{
		.gpio = 38,
		.settings = {
			[GPIOMUX_SUSPENDED] = &maxim_hs_detect,
		},
	},	
};
#endif
...
#if defined(CONFIG_LGE_FELICA_NFC)
static struct msm_gpiomux_config apq8064_felica_rev_c_configs[] __initdata = {
    /* FELICA PON */
	{
		.gpio = 37,
		.settings = {
...
static struct msm_gpiomux_config apq8064_felica_rev_b_configs[] __initdata = {
    /* FELICA PON */
	{
		.gpio = 37,
		.settings = {
...
static struct gpiomux_setting fuelgauge_max17047_cfg= {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_2MA,
	.pull = GPIOMUX_PULL_UP,
	.dir = GPIOMUX_IN,
...
static struct msm_gpiomux_config apq8064_fuel_gauge_configs[] __initdata = {
	{
		.gpio = 36,
		.settings = {
			[GPIOMUX_SUSPENDED] = &fuelgauge_max17047_cfg,
			},
	},
};
#endif
...
void __init apq8064_init_gpiomux(void)
{
	...
	msm_gpiomux_install(apq8064_gsbi1_i2c_8ma_configs,
       ...
       if (lge_get_board_revno() == HW_REV_B){
              msm_gpiomux_install(apq8064_headset_rev_b_configs,
			ARRAY_SIZE(apq8064_headset_rev_b_configs));
       }
#endif

#if defined(CONFIG_SII8334_MHL_TX)
	if (lge_get_board_revno() < HW_REV_C) {
		apq8064_sii8334_mhl_configs[0].gpio = GPIO_MHL_INT_N_REV_AB;
	} else {
		apq8064_sii8334_mhl_configs[0].gpio = GPIO_MHL_INT_N_REV_C;
	}
	msm_gpiomux_install(apq8064_sii8334_mhl_configs,
			ARRAY_SIZE(apq8064_sii8334_mhl_configs));
#endif /* CONFIG_SII8334_MHL_TX */
...
#if defined(CONFIG_LGE_FELICA_NFC)
  if (lge_get_board_revno() > HW_REV_B){
	msm_gpiomux_install(apq8064_felica_rev_c_configs,
			ARRAY_SIZE(apq8064_felica_rev_c_configs));
  }
	else{
	msm_gpiomux_install(apq8064_felica_rev_b_configs,
			ARRAY_SIZE(apq8064_felica_rev_b_configs));
  }
#endif
	...
	if(lge_get_board_revno() > HW_REV_A){ 
		...
		pr_debug("[MAX17047] gpiomux_install");
	}
#endif
