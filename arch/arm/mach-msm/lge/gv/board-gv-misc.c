...
#ifdef CONFIG_SII8334_MHL_TX
#define GPIO_MHL_RESET_N_REV_A 	62
#define GPIO_MHL_RESET_N_REV_BC	31
#define GPIO_MHL_INT_N_REV_AB	43
#define GPIO_MHL_INT_N_REV_C	32

static int sii8334_mhl_gpio_init(void)
{
	int rc;
	unsigned int gpio_mhl_reset_n = 0;
	unsigned int gpio_mhl_int_n = 0;
	hw_rev_type board_rev = lge_get_board_revno();

	switch(board_rev)
	{
	case HW_REV_A:
		gpio_mhl_int_n = GPIO_MHL_INT_N_REV_AB;
		gpio_mhl_reset_n = GPIO_MHL_RESET_N_REV_A;
		break;
	case HW_REV_B:
		gpio_mhl_int_n = GPIO_MHL_INT_N_REV_AB;
		gpio_mhl_reset_n = GPIO_MHL_RESET_N_REV_BC;
		break;
	case HW_REV_C:
	default:
		gpio_mhl_int_n = GPIO_MHL_INT_N_REV_C;
		gpio_mhl_reset_n = GPIO_MHL_RESET_N_REV_BC;
		break;
	}
...
struct i2c_board_info i2c_mhl_info[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(GPIO_MHL_INT_N_REV_C),
...
static struct i2c_registry i2c_mhl_devices __initdata = {
	I2C_SURF | I2C_FFA | I2C_RUMI | I2C_SIM | I2C_LIQUID | I2C_GV,
...
static void __init irrc_init(void)
{
    ...
    irrc_data.en_gpio = GPIO_RC_EN;

    printk("[IRRC] gpio_setting: %d\n",irrc_data.en_gpio);

    rc = gpio_request_one(VREG_3P0_IRDA_EN, GPIOF_OUT_INIT_LOW,"VREG_3P0_IRDA_EN");
    if (rc) {
	printk(KERN_ERR "%s: GPIO_RC_LEVEL_EN gpio_tlmm_config(%#x)=fg%d\n",
		__func__, VREG_3P0_IRDA_EN, rc);
	return;
    }
...
static void __init lge_add_i2c_mhl_device(void)
{
	...
	case HW_REV_A:
	case HW_REV_B:
		i2c_mhl_info[0].irq = MSM_GPIO_TO_INT(GPIO_MHL_INT_N_REV_AB);
		break;
	case HW_REV_C:
	default:
		i2c_mhl_info[0].irq = MSM_GPIO_TO_INT(GPIO_MHL_INT_N_REV_C);
		break;
	}
