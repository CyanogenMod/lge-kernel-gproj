...
static void __init lge_add_i2c_anx7808_device(void)
{
    ...
    anx7808_pdata.gpio_int =  ( lge_get_board_revno() == HW_REV_F ||
				 lge_get_board_revno() == HW_REV_B ||
                                lge_get_board_revno() == HW_REV_A ) ?
                                GPIO_SLIMPORT_INT_N_REV_OLD : GPIO_SLIMPORT_INT_N;
...
#define GPIO_RC_EN_REV_F    22
#define GPIO_RC_EN    PM8921_GPIO_PM_TO_SYS(23) //REV_E, REV_C

struct pm_gpio irrc_param = {
...
static struct uei_irrc_pdata_type irrc_data= {
       .en_gpio = GPIO_RC_EN,
       .en_state = 0,
};
static void __init irrc_init(void)
{
       int rc = 0;
       
       if(lge_get_board_revno() == HW_REV_F)  irrc_data.en_gpio = GPIO_RC_EN_REV_F;
       else if(lge_get_board_revno() == HW_REV_A || lge_get_board_revno() == HW_REV_B ) irrc_data.en_gpio = GPIO_RC_EN_REV_A_B;
       else irrc_data.en_gpio = GPIO_RC_EN;
       printk("[IRRC] gpio_setting: %d\n",irrc_data.en_gpio);

       rc = gpio_request(irrc_data.en_gpio, "RC_LEVEL_EN");
       if(rc) {
              printk(KERN_ERR "%s: rc_level_en %d request failed\n",
                            __func__, irrc_data.en_gpio);
              return;
       }

       if(lge_get_board_revno() == HW_REV_A || lge_get_board_revno() == HW_REV_B || lge_get_board_revno() == HW_REV_F){
              rc = gpio_tlmm_config(GPIO_CFG(irrc_data.en_gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
              if(rc){
                     printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=fg%d\n",
                                   __func__,irrc_data.en_gpio,rc);
                     return;
              }
       }else{
              rc= pm8xxx_gpio_config(irrc_data.en_gpio, &irrc_param);
       }
...
#define GPIO_POUCH_INT		7

static unsigned hall_ic_int_gpio[] = {GPIO_POUCH_INT};

static unsigned hall_ic_gpio[] = {
	GPIO_CFG(GPIO_POUCH_INT, 0, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
static void __init hall_ic_init(void)
{
	...
	for(i=0; i<ARRAY_SIZE(hall_ic_gpio); i++){
       ...
       printk(KERN_INFO "cradle init END");
...
#ifdef CONFIG_LGE_FTT_CHARGER
#define GPIO_FTT_CHIP_ENABLE		PM8921_GPIO_PM_TO_SYS(26) // CHG_STAT (EN1)
#define GPIO_FTT_DETECT			PM8921_GPIO_PM_TO_SYS(25) // WLC_ACTIVE (CHG_N)
#define GPIO_FTT_FREQUANCY		22 // WC_TRACK, GK_KR Rev.E(84)

static struct ftt_charger_pdata ftt_charger_data = {
	.en1	= GPIO_FTT_CHIP_ENABLE,
	.detect = GPIO_FTT_DETECT,
	.ftt	= GPIO_FTT_FREQUANCY,
};

static struct platform_device ftt_charger_device = {
...
//LGE_CHANGE_S, ar0804.han@lge.com vreg_l9 is used only for sensor
	...
	if (!vreg_l9) {
		    ...
		    if (IS_ERR(vreg_l9)) {
				 printk("%s: regulator get of 8921_l9 failed (%ld)\n", __func__, PTR_ERR(vreg_l9));
			     rc = PTR_ERR(vreg_l9);
			     vreg_l9 = NULL;
			}
	 }
...
//LGE_CHANGE_E, ar0804.han@lge.com
...
#ifdef CONFIG_LGE_IRRC
       irrc_init();
       platform_device_register(&uei_irrc_device);
#endif
	...
	if(lge_bd_rev != HW_REV_F) {
		if(lge_bd_rev != HW_REV_A && lge_bd_rev != HW_REV_B)			
			ftt_charger_data.ftt = 84;
		platform_device_register(&ftt_charger_device);
	}
#endif
}
