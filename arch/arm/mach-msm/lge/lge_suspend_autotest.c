/*
 * kernel/power/suspend_autotest.c - Suspend to RAM and standby autotest facility.
 *
...
static void suspend_autotest_late_resume(struct early_suspend *handler) {
	...
	case SUSPEND:
		mod_timer(&suspend_autotest_timer, jiffies + (HZ * TEST_POWEROFF_SECONDS));
